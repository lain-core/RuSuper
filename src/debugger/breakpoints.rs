...
mod tests {
    ...
    mod breakpoint_literal_tests {
        ...
        fn test_dbg_breakpoint_set() {
            ...
            for (test_input, expected_result) in zip(test_inputs, expected_numerics) {
                assert_eq!( (), dbg_breakpoint_set(test_input, &mut test_debug, &mut test_vm).unwrap());
    ...
    mod breakpoint_tag_tests {
        ...
        use breakpoints::parser::tests::testconst::{TEST_BASE_ADDR, TEST_HEX_VALUE, TEST_TAG_NAME, TEST_TAG_NAME2};
        ...
        fn test_dbg_breakpoint_set() {
            ...
            for (test_input, expected_result) in zip(test_inputs, numeric_results){
                if let Some(result) = expected_result {
                    ...
                    assert!(dbg_breakpoint_set(test_input, &mut test_debug, &mut test_vm).is_err());
                }
            }
        }
        ...
        fn test_dbg_breakpoint_set_with_existing(){ 
            ...
            for (test_input, expected_result) in zip(test_inputs, numeric_results){
                test_debug.tags.insert(TEST_TAG_NAME.to_string(), TEST_BASE_ADDR);
                test_debug.tags.insert(TEST_TAG_NAME2.to_string(), TEST_HEX_VALUE);
                if let Some(result) = expected_result {
